package net.wheel.cutils.api.module;
...
public class Module {
    ...
    public Module(String displayName, String[] alias, String desc, String key, int color, boolean hidden,
            boolean enabled, ModuleType type) {
    ...
    public TextComponentString toUsageTextComponent() {
        ...
        for (Value v : this.getValueList()) {
            ...
            if (v.getValue() instanceof Enum) {
                ...
                msg.appendSibling(
                        new TextComponentString(valuePrefix + v.getName() + ChatFormatting.GRAY + " <" + options + ">"
            ...
            if (v.getValue() instanceof Regex) {
                msg.appendSibling(new TextComponentString(valuePrefix + v.getName() + ChatFormatting.GRAY + " <regex>"
                        + ChatFormatting.RESET + ": " + v.getValue())
                                .setStyle(new Style().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                        new TextComponentString(v.getName() + "\n" + ChatFormatting.GOLD
                                                + ((v.getDesc() == null || v.getDesc().equals(""))
                                                        ? "There is no description for this regular expression value."
                                                        : v.getDesc())
                                                + ChatFormatting.RESET + "\n " + ChatFormatting.GRAY + "<regex>")))));
            }

            if (v.getValue() instanceof Shader) {
                msg.appendSibling(new TextComponentString(valuePrefix + v.getName() + ChatFormatting.GRAY + " <shader>"
                        + ChatFormatting.RESET + ": " + v.getValue())
                                .setStyle(new Style().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                        new TextComponentString(v.getName() + "\n" + ChatFormatting.GOLD
                                                + ((v.getDesc() == null || v.getDesc().equals(""))
                                                        ? "There is no description for this shader ID value."
                                                        : v.getDesc())
    ...
    public enum ModuleType {
        WAR, MVMT, RENDER, LOCAL, GLOBAL, CRACK, HIDDEN, UI, MISC
    }
}
