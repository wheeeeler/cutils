...
public class FileUtil {
    ...
    public static FileReader createReader(File file) {
        if (file.exists()) {
            try {
                ...
                crack.INSTANCE.getLogger().log(Level.WARNING, "Error creating reader: File not found exception");
            }
        }
        return null;
    }
    ...
    public static void closeReader(FileReader reader) {
        try {
            ...
            crack.INSTANCE.getLogger().log(Level.WARNING, "Error closing reader: IO exception");
        }
    }

    public static File createDirectory(String dir) {
        File folder = new File(crack.INSTANCE.getConfigManager().getConfigDir(), dir);
        if (!folder.exists())
            folder.mkdir();
        return folder;
    }
