...
public final class GLUProjection {
                        ...
                        if (GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection,
                            ...
                            if (opposite) {
                                ...
                                vecY = this.displayHeight * this.heightScale
                                        - (displayHeight - (double) this.coords.get(1)) * this.heightScale
    ...
    public float[] projectBox(AxisAlignedBB bb, ClampMode clampMode, boolean extrudeInverted) {
        Projection[] projections = new Projection[8];
        projections[0] = this.project(bb.minX, bb.minY, bb.minZ, clampMode, extrudeInverted);
        projections[1] = this.project(bb.maxX, bb.minY, bb.minZ, clampMode, extrudeInverted);
        projections[2] = this.project(bb.minX, bb.maxY, bb.minZ, clampMode, extrudeInverted);
        projections[3] = this.project(bb.maxX, bb.maxY, bb.minZ, clampMode, extrudeInverted);
        projections[4] = this.project(bb.minX, bb.minY, bb.maxZ, clampMode, extrudeInverted);
        projections[5] = this.project(bb.maxX, bb.minY, bb.maxZ, clampMode, extrudeInverted);
        projections[6] = this.project(bb.minX, bb.maxY, bb.maxZ, clampMode, extrudeInverted);
        projections[7] = this.project(bb.maxX, bb.maxY, bb.maxZ, clampMode, extrudeInverted);
        ...
        for (Projection proj : projections) {
            if (proj.isType(Projection.Type.FAIL)) {
                return null;
            }
            minX = Math.min(minX, (float) proj.getX());
            minY = Math.min(minY, (float) proj.getY());
            maxX = Math.max(maxX, (float) proj.getX());
            maxY = Math.max(maxY, (float) proj.getY());
        }
