...
public final class RenderUtil {
    ...
    public static void updateModelViewProjectionMatrix() {
        ...
        GLUProjection.getInstance().updateMatrices(VIEWPORT, MODELVIEW, PROJECTION,
                (float) res.getScaledWidth() / (float) Minecraft.getMinecraft().displayWidth,
    ...
    public static void drawText(float[] bounds, String text, int color, int yOffset) {
        Minecraft mc = Minecraft.getMinecraft();
        float midX = (bounds[0] + bounds[2]) / 2.0f;
        float y = bounds[1] + yOffset;
        mc.fontRenderer.drawStringWithShadow(text, midX - mc.fontRenderer.getStringWidth(text) / 2.0f, y, color);
    }

    public static void drawSideGradientRect(float left, float top, float right, float bottom, int startColor,
            int endColor) {
        float f = (float) (startColor >> 24 & 255) / 255.0F;
        float f1 = (float) (startColor >> 16 & 255) / 255.0F;
        float f2 = (float) (startColor >> 8 & 255) / 255.0F;
    ...
    public static void drawBorderedRect(float x, float y, float x1, float y1, float lineWidth, int insideColor,
            int borderColor) {
    ...
    public static void drawBorderedRectBlurred(float x, float y, float x1, float y1, float lineWidth, int insideColor,
            int borderColor) {
    ...
    public static void drawLine3D(double x, double y, double z, double x1, double y1, double z1, float thickness,
            int hex) {
        float red = (hex >> 16 & 0xFF) / 255.0F;
        float green = (hex >> 8 & 0xFF) / 255.0F;
        float blue = (hex & 0xFF) / 255.0F;
    ...
    public static void glScissor(float x, float y, float x1, float y1, final ScaledResolution sr) {
        GL11.glScissor((int) (x * sr.getScaleFactor()),
                (int) (Minecraft.getMinecraft().displayHeight - (y1 * sr.getScaleFactor())),
                (int) ((x1 - x) * sr.getScaleFactor()), (int) ((y1 - y) * sr.getScaleFactor()));
    }

    public static void glBillboard(float x, float y, float z) {
        float scale = 0.016666668f * 1.6f;
        GlStateManager.translate(x - Minecraft.getMinecraft().getRenderManager().renderPosX,
    ...
    public static void drawTexture(float x, float y, float textureX, float textureY, float width, float height) {
        ...
        bufferbuilder.pos((x + width), (y + height), 0.0D).tex(((textureX + width) * f), ((textureY + height) * f1))
                .endVertex();
