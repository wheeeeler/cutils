...
public final class BlocksComponent extends HudComponent {
    ...
    public BlocksComponent(Value<List<Block>> value) {
        ...
        for (Block block : Block.REGISTRY) {
            ...
            if (!(block instanceof BlockLiquid || block instanceof IFluidBlock) && Minecraft.getMinecraft()
                    .getRenderItem().getItemModelMesher().getItemModel(new ItemStack(block)) == Minecraft.getMinecraft()
                            .getRenderItem().getItemModelMesher().getModelManager().getMissingModel()) {
                continue;
            }
                    ...
                    if (block instanceof BlockLiquid || block instanceof IFluidBlock) {
                        ...
                        if (fluidStill == null) {
                            ...
                            sprite = Minecraft.getMinecraft().getTextureMapBlocks()
                                    .getTextureExtry(fluidStill.toString());
                        }

                        RenderUtil.glColor(fluid.getColor());

                        RenderUtil.drawTexture(rectX + 1, rectY + 1, 14, 14, sprite.getMinU(), sprite.getMinV(),
                                sprite.getMaxU(), sprite.getMaxV());
                    } else {
                        final ItemStack itemStack = new ItemStack(block);
                        Minecraft.getMinecraft().getRenderItem().renderItemIntoGUI(itemStack,
                                (int) renderPaddingX + (int) this.getX() + xOffset,
                                (int) renderPaddingY + (int) this.getY() + yOffset);
                        Minecraft.getMinecraft().getRenderItem().renderItemOverlayIntoGUI(
                                Minecraft.getMinecraft().fontRenderer, itemStack,
                                (int) renderPaddingX + (int) this.getX() + xOffset,
                                (int) renderPaddingY + (int) this.getY() + yOffset, null);
                    }
                }
    ...
    public void mouseRelease(int mouseX, int mouseY, int button) {
        ...
        if (this.searchBox.focused) {
            ...
            if (this.isMouseInside(mouseX, mouseY) && mouseY > this.getY() + renderPaddingY) {
                ...
                for (Block block : this.displayedBlocks) {
                    if (yOffset <= 56) {
                        if (mouseX > renderPaddingX + this.getX() + xOffset
                                && mouseX < renderPaddingX + this.getX() + xOffset + 16
                                && mouseY > renderPaddingY + this.getY() + yOffset
                                && mouseY < renderPaddingY + this.getY() + yOffset + 16) {
                            if (this.value.getValue().contains(block))
                                this.value.getValue().remove(block);
                            else
                                this.value.getValue().add(block);

                            crack.INSTANCE.getEventManager().dispatchEvent(new EventUIListValueChanged(this.value));
                        }
                    }
    ...
    public void keyTyped(char typedChar, int keyCode) {
        ...
        if (this.searchBox.getText().isEmpty() && this.displayedBlocks.size() != 0) {
