...
public final class ItemsComponent extends HudComponent {
    ...
    public ItemsComponent(Value<List<Item>> value) {
        ...
        for (Item item : Item.REGISTRY) {
            ...
            if (Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
                    .getItemModel(new ItemStack(item)) == Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
        ...
        if (this.items.size() > 0 && this.searchBox.focused) {
            ...
            for (Item item : this.displayedItems) {
                if (yOffset <= 56) {
                    ...
                    RenderUtil.drawBorderedRect(renderPaddingX + this.getX() + xOffset,
                            renderPaddingY + this.getY() + yOffset, renderPaddingX + this.getX() + xOffset + 16,
                            renderPaddingY + this.getY() + yOffset + 16, 0.5f, color, borderColor);
    ...
    public void keyTyped(char typedChar, int keyCode) {
        ...
        if (this.searchBox.getText().isEmpty() && this.displayedItems.size() != 0) {
