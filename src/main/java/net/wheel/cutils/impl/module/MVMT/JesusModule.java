...
public final class JesusModule extends Module {
    ...
    public final Value<Mode> mode = new Value<Mode>("Mode", new String[] { "Mode", "M" },
            "The current Jesus/WaterWalk mode to use", Mode.NCP);
    public final Value<Float> offset = new Value<Float>("Offset", new String[] { "Off", "O" },
            "Amount to offset the player into the water's bounding box", 0.05f, 0.0f, 0.9f, 0.01f);

    public JesusModule() {
        super("WaterWalk", new String[] { "LiquidWalk", "WaterWalk" }, "Allows you to walk on water", "NONE", -1,
                ModuleType.MVMT);
    }
    ...
    public void getLiquidCollisionBB(EventLiquidCollisionBB event) {
        if (Minecraft.getMinecraft().world != null && Minecraft.getMinecraft().player != null) {
            if (this.checkCollide() && !(Minecraft.getMinecraft().player.motionY >= 0.1f)
                    && event.getBlockPos().getY() < Minecraft.getMinecraft().player.posY - this.offset.getValue()) {
    ...
    public void updateWalkingPlayer(EventUpdateWalkingPlayer event) {
        if (event.getStage() == EventStageable.EventStage.PRE) {
            if (!Minecraft.getMinecraft().player.isSneaking() && !Minecraft.getMinecraft().player.noClip
                    && !Minecraft.getMinecraft().gameSettings.keyBindJump.isKeyDown() && isInLiquid()) {
    ...
    public void sendPacket(EventSendPacket event) {
        if (event.getStage() == EventStageable.EventStage.PRE) {
            if (event.getPacket() instanceof CPacketPlayer) {
                if (this.mode.getValue() != Mode.VANILLA && Minecraft.getMinecraft().player.getRidingEntity() == null
                    ...
                    if (!isInLiquid() && isOnLiquid(this.offset.getValue()) && checkCollide()
                            && Minecraft.getMinecraft().player.ticksExisted % 3 == 0) {
