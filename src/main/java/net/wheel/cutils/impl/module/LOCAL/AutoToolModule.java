...
public final class AutoToolModule extends Module {
    ...
    public final Value<Boolean> silent = new Value<>("Silent", new String[] { "Sil" },
            "Hold any item and spoof your mining tool. (Best tool must be in inventory)", false);
    public BlockPos position;
    public EnumFacing facing;
    private boolean send;

    public AutoToolModule() {
        super("AutoTool", new String[] { "Tool" }, "Automatically switches to the best tool", "NONE", -1,
                ModuleType.LOCAL);
    }
    ...
    private float getDigSpeed(IBlockState state, BlockPos pos, ItemStack stack) {
        ...
        if (mc.player.isPotionActive(MobEffects.HASTE)) {
            f *= 1.0F
                    + (float) (Objects.requireNonNull(mc.player.getActivePotionEffect(MobEffects.HASTE)).getAmplifier()
                            + 1) * 0.2F;
        }
    ...
    public void damageBlock(EventPlayerDamageBlock event) {
        ...
        if (this.silent.getValue()) {
            ...
            if (slot != -1) {
                mc.playerController.curBlockDamageMP += blockStrength(event.getPos(),
                        mc.player.inventoryContainer.getSlot(slot).getStack());
            } else {
                final int hotbar = getToolHotbar(event.getPos());
                if (hotbar != -1) {
                    mc.playerController.curBlockDamageMP += blockStrength(event.getPos(),
                            mc.player.inventory.getStackInSlot(hotbar));
                }
            }
        } else {
                    ...
                    if (this.position != null) {
                        ...
                        if (slot != -1) {
                            ...
                            Minecraft.getMinecraft().playerController.windowClick(
                                    Minecraft.getMinecraft().player.inventoryContainer.windowId, slot,
                                    Minecraft.getMinecraft().player.inventory.currentItem, ClickType.SWAP,
                                    Minecraft.getMinecraft().player);
                            send = true;
                            Minecraft.getMinecraft().player.connection.sendPacket(new CPacketPlayerDigging(
                                    CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.position, this.facing));
                            Minecraft.getMinecraft().playerController.windowClick(
                                    Minecraft.getMinecraft().player.inventoryContainer.windowId, slot,
                                    Minecraft.getMinecraft().player.inventory.currentItem, ClickType.SWAP,
                                    Minecraft.getMinecraft().player);
                            ...
                            if (hotbar != -1) {
                                event.setCanceled(true);
                                Minecraft.getMinecraft().playerController.windowClick(
                                        Minecraft.getMinecraft().player.inventoryContainer.windowId, hotbar,
                                        Minecraft.getMinecraft().player.inventory.currentItem, ClickType.SWAP,
                                        Minecraft.getMinecraft().player);
                                send = true;
                                Minecraft.getMinecraft().player.connection.sendPacket(new CPacketPlayerDigging(
