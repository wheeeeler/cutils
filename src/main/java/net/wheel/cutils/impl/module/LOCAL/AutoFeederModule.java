...
public final class AutoFeederModule extends Module {

    public final Value<Float> hunger = new Value<Float>("Hunger", new String[] { "food", "h" },
            "The amount of hunger needed to acquire some food", 9.0f, 0.0f, 20.0f, 0.5f);
    public final Value<Integer> forcedSlot = new Value<Integer>("Slot", new String[] { "s" },
            "The hot-bar slot to put the food into (45 for offhand)", 43, 0, 43, 1);
    public final Value<Boolean> gapples = new Value<Boolean>("Gapples",
            new String[] { "gap", "gapple", "goldenapple", "goldenapples" }, "Allow eating golden apples for food",
            false);
    ...
    public AutoFeederModule() {
        super("AutoFeeder", new String[] { "Eat", "AutoFeed" },
                "Automatically swaps & eats food when hunger is below the set threshold", "NONE", -1, ModuleType.LOCAL);
    }
    ...
    public void onPlayerUpdate(EventPlayerUpdate event) {
        ...
        if (this.foodSlot != -1) {
            if (this.forcedSlot.getValue() != 45) {
                ...
                if (this.foodSlot < 36) {
                    mc.playerController.windowClick(0, this.forcedSlot.getValue(), 0, ClickType.QUICK_MOVE, mc.player);
                    ...
                    mc.player.inventory.currentItem = this.foodSlot - 36;
                }
            } else {
    ...
    private boolean isFoodItem(Item item) {
        ...
        return item != Items.GOLDEN_APPLE && item != Items.CHORUS_FRUIT && item != Items.ROTTEN_FLESH
