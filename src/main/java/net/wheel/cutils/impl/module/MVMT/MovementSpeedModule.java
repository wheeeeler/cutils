...
public final class MovementSpeedModule extends Module {

    public final Value<Mode> mode = new Value<Mode>("Mode", new String[] { "Mode", "M" }, "The speed mode to use",
            Mode.VANILLA);
    public final Value<Float> speed = new Value<Float>("Speed", new String[] { "Spd", "Amount", "A", "S" },
            "Speed multiplier, higher numbers equal faster motion", 0.1f, 0.0f, 10.0f, 0.1f);
    private int tick;
    private double prevDistance;
    private double movementSpeed;

    public MovementSpeedModule() {
        super("MovementSpeed", new String[] { "Spd" }, "Allows you to move faster", "NONE", -1, ModuleType.MVMT);
    }
        ...
        if (this.mode.getValue() == Mode.BHOP) {
            ...
            if ((this.tick == 2) && ((mc.player.moveForward != 0.0F) || (mc.player.moveStrafing != 0.0F))) {
                ...
                if ((!mc.world.getCollisionBoxes(mc.player,
                        mc.player.getEntityBoundingBox().offset(0.0D, mc.player.motionY, 0.0D)).isEmpty())
