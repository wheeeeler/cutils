...
public final class ChunkGenModule extends Module {

    public final Value<Mode> mode = new Value<Mode>("Mode", new String[] { "M", "type", "t" },
            "Select a mode to use to draw the chunk visual", Mode.PLANE);
    public final Value<Color> color = new Value<Color>("Color", new String[] { "color", "c" },
            "Change the color of the chunk visual", new Color(255, 255, 255));
    public final Value<Integer> alpha = new Value<Integer>("Alpha", new String[] { "Alp", "Opacity", "a", "o" },
            "Edit the alpha of the chunk visual", 127, 1, 255, 1);
    public final Value<Float> lineWidth = new Value<Float>("LineWidth",
            new String[] { "Width", "Line-Width", "line", "size", "s", "l" }, "Edit the line width chunk visual", 1.5f,
            0.1f, 5.0f, 0.1f);
    public final Value<Float> height = new Value<Float>("Height", new String[] { "H" },
            "Edit the height of the chunk visual", 3f, 1f, 25f, 1f);
    private final ICamera frustum = new Frustum();
    private final List<ChunkData> chunkDataList = new ArrayList<>();
    private Color currentColor;

    public ChunkGenModule() {
        super("ChunkGenHighlight", new String[] { "ChunkGen" }, "Highlights newly generated chunks", "NONE", -1,
                ModuleType.RENDER);
        this.currentColor = new Color(this.color.getValue().getRed(), this.color.getValue().getGreen(),
                this.color.getValue().getBlue(), this.alpha.getValue());
    }
    ...
    public void render3D(EventRender3D event) {
        ...
        for (int i = this.chunkDataList.size() - 1; i >= 0; i--) {
            ...
            if (chunkData != null) {
                ...
                final AxisAlignedBB bb = new AxisAlignedBB(chunkData.x, 0, chunkData.z, chunkData.x + 16, 1,
                        chunkData.z + 16);
                ...
                if (frustum.isBoundingBoxInFrustum(bb)) {
                    ...
                    switch (this.mode.getValue()) {
                        case BOX:
                            RenderUtil.drawFilledBox(x, y, z, chunkBB,
                                    ColorUtil.changeAlpha(color, this.alpha.getValue()));
                            break;
                        case OUTLINE:
