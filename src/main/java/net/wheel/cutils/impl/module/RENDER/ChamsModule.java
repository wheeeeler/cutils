...
import net.wheel.cutils.api.event.render.EventDrawNameplate;
...
public final class ChamsModule extends Module {
    ...
    public final Value<Mode> mode = new Value<Mode>("Mode", new String[] { "Mode" }, "The chams mode to use",
            Mode.NORMAL);
    ...
    public final Value<Color> playersColor = new Value<Color>("PlayersColor", new String[] { "playerscolor", "pc" },
            "Change the color of players on chams", new Color(255, 68, 68));
    ...
    public final Value<Color> mobsColor = new Value<Color>("MobsColor", new String[] { "mobscolor", "mc" },
            "Change the color of mobs on chams", new Color(255, 170, 0));
    ...
    public final Value<Color> animalsColor = new Value<Color>("AnimalsColor", new String[] { "animalscolor", "ac" },
            "Change the color of animals on chams", new Color(0, 255, 68));
    ...
    public final Value<Color> crystalsColor = new Value<Color>("CrystalsColor",
            ...
            "Change the color of ender crystals on chams", new Color(205, 0, 205));

    public final Value<Boolean> unknowns = new Value<Boolean>("Unknowns", new String[] { "unknown", "unknowns" },
            "Choose to enable on unknown entity types", false);
    public final Value<Color> unknownsColor = new Value<Color>("UnknownsColor",
            new String[] { "unknownscolor", "unknowncolor", "uc" }, "Change the color of unknown entity types on chams",
            new Color(60, 60, 60));
    ...
    public final Value<Shader> shader = new Value<Shader>("Shader",
            new String[] { "shader", "program", "shaderprogram", "sp" }, "Change the shader to use when in shader mode",
            new Shader("resource:///assets/cutils/shaders/chams.json"));
    ...
    public ChamsModule() {
        super("ChamsModule", new String[] { "Cham", "Chameleon" }, "Allows you to see entities through walls", "NONE",
                -1,
                ModuleType.RENDER);
    }
        ...
        if (this.checkFilter(entityType)) {
            if (mode.equalsIgnoreCase("shader")) {
                if (event.getStage() == EventStageable.EventStage.PRE) {
                    this.queuedEntities.put(event.getEntity(), new QueuedEntity(event, entityType,
                            Minecraft.getMinecraft().getRenderManager().isRenderShadow()));
                }
    ...
    public void render3D(EventRender3D event) {
        ...
        for (HashMap.Entry<Entity, QueuedEntity> entry : this.queuedEntities.entrySet()) {
            ...
            if (qEntity.render != null) {
                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0f, 240.0f);
                qEntity.render.doRenderShadowAndFire(entity, qEntity.x, qEntity.y, qEntity.z, qEntity.yaw,
                        qEntity.partialTicks);
            }
        }
        ...
        for (HashMap.Entry<Entity, QueuedEntity> entry : this.queuedEntities.entrySet()) {
            ...
            if (qEntity.render != null) {
                ...
                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float) (lightLevel % 65536),
                        (float) (lightLevel / 65536));
                GlStateManager.color(1.0f, 1.0f, 1.0f);
        ...
        for (EventDrawNameplate nEvent : this.queuedNameplates) {
            EntityRenderer.drawNameplate(nEvent.fontRenderer, nEvent.str, nEvent.x, nEvent.y, nEvent.z,
                    nEvent.verticalShift, nEvent.viewerYaw, nEvent.viewerPitch, nEvent.isThirdPersonFrontal,
                    nEvent.isSneaking);
        }
    ...
    private EntityType getEntityType(Entity entity) {
        if (entity == Minecraft.getMinecraft().player ||
            ...
            } else if (crack.INSTANCE.getFriendManager().isFriend(entity) != null) {
    ...
    private class QueuedEntity {
        ...
        public QueuedEntity(Entity entity, double x, double y, double z, float yaw, float partialTicks,
                EntityType entityType, boolean hasShadow) {
            this.x = x;
            this.y = y;
            this.z = z;
            this.yaw = yaw;
        ...
        public QueuedEntity(EventRenderEntity event, EntityType entityType, boolean hasShadow) {
            this(event.getEntity(), event.getX(), event.getY(), event.getZ(), event.getYaw(), event.getPartialTicks(),
                    entityType, hasShadow);
        }
    }
