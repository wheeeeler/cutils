...
public final class TracerModule extends Module {
    ...
    public final Value<Color> playersColor = new Value<Color>("PlayersColor", new String[] { "playerscolor", "pc" },
            "Change the color of player tracer lines", new Color(255, 68, 68));
    ...
    public final Value<Color> mobsColor = new Value<Color>("MobsColor", new String[] { "mobscolor", "mc" },
            "Change the color of mob tracer lines", new Color(255, 170, 0));
    ...
    public final Value<Color> animalsColor = new Value<Color>("AnimalsColor", new String[] { "animalscolor", "ac" },
            "Change the color of animal tracer lines", new Color(0, 255, 68));
    ...
    public final Value<Color> vehiclesColor = new Value<Color>("VehiclesColor", new String[] { "vehiclescolor", "vc" },
            "Change the color of vehicle tracer lines", new Color(213, 255, 0));
    ...
    public final Value<Color> itemsColor = new Value<Color>("ItemsColor", new String[] { "itemscolor", "ic" },
            "Change the color of item tracer lines", new Color(0, 255, 170));

    public final Value<Mode> mode = new Value<Mode>("Mode", new String[] { "Mode" },
            "The rendering mode to use for drawing the tracer-line", Mode.TWO_D);

    public final Value<Color> friendsColor = new Value<Color>("FriendsColor", new String[] { "friendscolor", "fc" },
            "Change the color of added friends tracer lines", new Color(153, 0, 238));
    public final Value<Float> width = new Value<Float>("Width", new String[] { "Wid" },
            "Pixel width of each tracer-line", 0.5f, 0.0f, 5.0f, 0.1f);
    public final Value<Integer> alpha = new Value<Integer>("Alpha", new String[] { "Alpha", "A", "Opacity", "Op" },
            "Alpha value for each drawn line", 255, 1, 255, 1);

    public TracerModule() {
        super("TracerModule", new String[] { "Trace", "Tracer", "Snapline", "Snaplines" }, "Draws a line to entities",
                ...
                if (e != null) {
                    if (this.checkFilter(e)) {
                        ...
                        final GLUProjection.Projection projection = GLUProjection.getInstance().project(
                                pos.x - mc.getRenderManager().viewerPosX, pos.y - mc.getRenderManager().viewerPosY,
                                pos.z - mc.getRenderManager().viewerPosZ, GLUProjection.ClampMode.NONE, true);
                        RenderUtil.drawLine((float) projection.getX(), (float) projection.getY(),
                                (float) event.getScaledResolution().getScaledWidth() / 2.0f,
                                (float) event.getScaledResolution().getScaledHeight() / 2.0f, this.width.getValue(),
                                this.getColor(e));
                    }
                }
            }
    ...
    public void render3D(EventRender3D event) {
        if (this.mode.getValue() == Mode.THREE_D) {
            ...
            for (Entity e : mc.world.loadedEntityList) {
                if (e != null) {
                    if (this.checkFilter(e)) {
                        final Vec3d pos = MathUtil.interpolateEntity(e, event.getPartialTicks()).subtract(
                        ...
                        RenderUtil.drawLine3D(forward.x, forward.y, forward.z, pos.x, pos.y, pos.z,
                                this.width.getValue(), this.getColor(e));
                    }
                }
            }
            RenderUtil.end3D();
    ...
    private boolean checkFilter(Entity entity) {
        boolean ret = this.players.getValue() && entity instanceof EntityPlayer
