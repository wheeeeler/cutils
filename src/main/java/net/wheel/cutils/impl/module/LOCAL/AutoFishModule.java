...
public final class AutoFishModule extends Module {
    ...
    public final Value<Boolean> durability = new Value<>("Durability", new String[] { "dura", "durabilitycheck", "d" },
            "Saves the current rod before it breaks", true);
    public final Value<Integer> durabilityLimit = new Value<>("DurabilityLimit",
            new String[] { "duralimit", "limit", "dl" }, "Minimum durability to stop using a rod", 5, 1, 10, 1);
    public final Value<Boolean> swap = new Value<>("Swap", new String[] { "autoswap", "newrod", "s" },
            "If the current rod is almost broken, swaps to a new rod", true);
    public final Value<Integer> swapSlot = new Value<Integer>("Slot", new String[] { "s" },
            "The hot-bar slot to put the fishing rod into (45 for offhand)", 43, 0, 45, 1);

    public AutoFishModule() {
        super("AutoFish", new String[] { "AutomaticFish" }, "Automatically catches fish and recasts", "NONE", -1,
                ModuleType.LOCAL);
    }
    ...
    private void doSwap(final EntityPlayerSP player, final PlayerControllerMP playerController) {
        ...
        if (bestSlot != -1) {
            if (this.swapSlot.getValue() != 45) {
                if (bestSlot < 36) {
                    playerController.windowClick(0, this.swapSlot.getValue(), 0, ClickType.QUICK_MOVE, player);
                ...
                playerController.windowClick(0, 45, 0, ClickType.QUICK_MOVE, player);
                playerController.windowClick(0, bestSlot, 0, ClickType.PICKUP, player);
